...
export interface MismatchFileOptions {
  /**
...
export class MismatchFile {
  ...
  constructor(public actualPath: string, public actual: string, public expectedPath: string, public expected: string, options: MismatchFileOptions) {
    const singleLine = actual.indexOf('\n') === -1 && expected.indexOf('\n') === -1

    this.diff = singleLine ?
      jsdiff.diffWords(actual, expected) :
      jsdiff.diffLines(
        actual && !actual.endsWith('\n') ? actual + '\n' : actual,
        expected && !expected.endsWith('\n') ? expected + '\n' : expected)
    this.formattedDiff = singleLine ?
      formatWordsDiff(this.diff) :
      formatLinesDiff(this.diff, options)
  }
  tersify() {
...
function formatLinesDiff(diff: jsdiff.IDiffResult[], options: MismatchFileOptions) {
